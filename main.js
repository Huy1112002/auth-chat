...
io.on('connection', async (socket) => {
    ...
    socket.on('chat message', data => {
        ...
        db.query("SELECT usenames FROM users_id WHERE socket_id = " + data.socket_id, function (err, result) {
            db.query("INSERT INTO messages VALUES (" + decoded.username + ", " + result[0] + "," + data.message + ")");
        });
    });
});
